...
def start_openvoice_teacher(config: VoiceConfig) -> None:
    """
    Start the OpenVoice teacher model service using Docker Compose.
    ...
    model_cache_dir = project_root / "models" / "shared" / "openvoice_baseline"
    ensure_dir(model_cache_dir)

    # Check if model checkpoints exist
    checkpoint_dir = model_cache_dir / "checkpoints_v2"
    converter_dir = checkpoint_dir / "converter"
    converter_cfg = converter_dir / "config.json"
    converter_ckpt = converter_dir / "checkpoint.pth"

    if converter_cfg.exists() and converter_ckpt.exists():
        logger.info(f"Found cached OpenVoice checkpoints at {checkpoint_dir}")
    else:
        logger.warning(
            f"OpenVoice checkpoints not found at {checkpoint_dir}. "
            "Please ensure checkpoints_v2_0417.zip is extracted to this location."
        )

    # Generate .env file with environment variables
    env_file = project_root / f".env.{config.voice_id}.openvoice"
    ...
    with open(env_file, "w") as f:
        ...
        f.write(f"OPENVOICE_PORT={teacher.port}\n")
        f.write(f"OPENVOICE_DEVICE={teacher.device}\n")
        f.write(f"OPENVOICE_LANGUAGE={teacher.language}\n")
        f.write(f"OPENVOICE_BASE_SPEAKER_KEY={teacher.base_speaker_key}\n")
        f.write(f"REFERENCE_AUDIO_DIR={reference_dir_abs}\n")
        f.write(f"MODEL_CACHE_DIR={model_cache_dir_abs}\n")

    # Get docker-compose file path
    compose_file = project_root / "docker" / "docker-compose.openvoice.yml"
    ...
    max_retries = 60  # Increased to 2 minutes for model initialization
    retry_delay = 2

    logger.info("Waiting for teacher model service to be ready (this may take a few minutes on first run)...")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                base_speaker = health_data.get("base_speaker", "unknown")
        ...
        if attempt < max_retries - 1:
            ...
            try:
                result = subprocess.run(
                    ["docker", "ps", "-a", "--filter", f"name={config.voice_id}_openvoice_teacher", "--format", "{{.Status}}"],
                    capture_output=True,
                    text=True,
                    check=True,
                )
            ...
            raise RuntimeError(
                ...
                f"Check container logs: docker logs {config.voice_id}_openvoice_teacher"
            )


def stop_openvoice_teacher(config: VoiceConfig) -> None:
    """
    Stop the OpenVoice teacher model service.
    ...
    teacher = config.synthetic.teacher
    logger.info(f"Stopping OpenVoice teacher model service for {config.voice_id}...")
...
def is_openvoice_teacher_running(config: VoiceConfig) -> bool:
    """
    Check if the OpenVoice teacher model service is running.
